...
class SEOService
{
        ...
        return match ($page) {
            'home' => SchemaCollection::make()
                ->add(fn (SEOData $d) => [
                    ...
                    'name'     => 'Lucide Inkt',
                    'url'      => route('home'),
                    'logo'     => secure_url('images/google_logo.png'),
                    'sameAs'   => [],
                    'description' => $config['description'] ?? null,
                ])
                ->add(fn (SEOData $d) => [
            ...
            'saidnursi', 'risale', 'herzameling' => SchemaCollection::make()
                ->add(fn (SEOData $d) => [
                    ...
                    'publisher'        => [
                        ...
                        'logo'  => [
                            '@type' => 'ImageObject',
                            'url'   => secure_url('images/google_logo.png'),
                        ],
                    ],
                    'isPartOf' => [
                        '@type' => 'WebSite',
    ...
    private static function getPageConfig(string $page): array
    {
        $pages = [
            ...
            'risale' => [
                'title'       => 'Wat is de Risale-i Nur? | Geloofswaarheden | Lucide Inkt',
                'description' => 'Wat is de Risale-i Nur? Geschreven door Bediüzzaman Said Nursi, waarin geloofswaarheden met rationele en diepgaande bewijzen worden uitgelegd.',
                'url'         => route('risale'),
                'image'       => secure_url('images/stapel_new_social.jpg'),
                'type'        => 'article',
            ],
            'herzameling' => [
                'title'       => 'Leven na de Dood — Bestaat er een Hiernamaals? | Risale-i Nur | Lucide Inkt',
                'description' => 'Bestaat er leven na de dood? Het Traktaat over de Herzameling van Said Nursi bewijst met rationele en Qur\'anische argumenten dat het hiernamaals noodzakelijk is.',
            ...
            'shop' => [
                'title'       => 'Risale-i Nur Boeken Kopen | Winkel | Lucide Inkt',
                'description' => 'Risale-i Nur boeken kopen? Ontdek onze collectie Nederlandse en Engelse vertalingen van Said Nursi. Bestel eenvoudig en veilig online.',
                'url'         => route('shop'),
    ...
    public static function getArtikelSEO(Artikel $artikel): SEOData
    {
        ...
        // OG Image priority:
        // 1. Dedicated og_image field (stored in seo/og/artikelen/ — guaranteed accessible)
        // 2. Featured image via Storage facade
        // 3. Site default
        if (!empty($artikel->og_image)) {
            $ogImage = Storage::disk('public')->url($artikel->og_image);
        } elseif ($artikel->featured_image) {
