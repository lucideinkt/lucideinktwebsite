    ...
    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
      ...
      <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
        <div class="w-full md:w-1/2">
          <form id="search-form" method="GET" action="{{ route('productCopyIndex') }}" class="flex items-center">
        ...
        <div class="w-full md:w-auto flex items-center justify-end">
          <a href="{{ route('productCopyCreatePage') }}"
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            ...
            <tr class="border-b border-gray-200 dark:border-gray-700">
              ...
              <td class="px-4 py-2 text-xs">{{ $productCopy->created_at->format('d-m-Y') }}</td>
              <td class="px-4 py-2 whitespace-nowrap">
                <div class="flex items-center gap-2">
                  <a href="{{ route('productCopyEditPage', $productCopy->id) }}"
                    class="text-xs font-medium text-primary-700 hover:underline dark:text-primary-400">Bewerken</a>
                  <form action="{{ route('productCopyDelete', $productCopy->id) }}" method="POST"
                    onsubmit="return confirm('Weet je zeker dat je {{ addslashes($productCopy->name) }} wilt verwijderen?')">
                    @csrf
                    @method('DELETE')
            ...
            @empty
            <tr>
              <td colspan="4" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">Geen exemplaren gevonden.</td>
            </tr>
            @endforelse
          </tbody>
        </table>
      ...
      <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
        <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
          @if($productCopies->total() > 0)
            Toont <span class="font-semibold text-gray-900 dark:text-white">{{ $productCopies->firstItem() }}-{{ $productCopies->lastItem() }}</span>
            van <span class="font-semibold text-gray-900 dark:text-white">{{ $productCopies->total() }}</span>
          @else
            <span class="font-semibold text-gray-900 dark:text-white">0</span> resultaten
          @endif
        </span>
        @if($productCopies->hasPages())
        <ul class="inline-flex items-stretch -space-x-px">
          <li>
            @if($productCopies->onFirstPage())
            ...
            @else
              <a href="{{ $productCopies->previousPageUrl() }}" class="flex items-center justify-center h-full py-1.5 px-3 ml-0 text-gray-500 bg-white rounded-l-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
              </a>
            @endif
          </li>
          @foreach($productCopies->getUrlRange(1, $productCopies->lastPage()) as $page => $url)
            <li>
              @if($page == $productCopies->currentPage())
          ...
          @endforeach
          <li>
            @if($productCopies->hasMorePages())
              <a href="{{ $productCopies->nextPageUrl() }}" class="flex items-center justify-center h-full py-1.5 px-3 leading-tight text-gray-500 bg-white rounded-r-lg border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
