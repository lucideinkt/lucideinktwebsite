...
<nav class="flex mb-4" aria-label="Breadcrumb">
  <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
    <li class="inline-flex items-center">
      <a href="{{ route('productCopyIndex') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-primary-600 dark:text-gray-400 dark:hover:text-white">
    ...
    <li aria-current="page">
      <div class="flex items-center">
        ...
        <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">{{ $productCopy->name }}</span>
      </div>
    </li>
  </ol>
</nav>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
  <div class="lg:col-span-2">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
      <div class="p-4 border-b border-gray-200 dark:border-gray-700">
        <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Productexemplaar bewerken</h2>
      </div>
      <form id="copy-edit-form" action="{{ route('productCopyUpdate', $productCopy->id) }}" method="POST" class="p-4 space-y-4">
        @method('PUT')
        @csrf

        <div>
          <label for="name" class="block mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Naam <span class="text-red-500">*</span></label>
          <input type="text" name="name" id="name" value="{{ old('name', $productCopy->name) }}"
        ...
        <div class="flex items-center gap-3 pt-2 border-t border-gray-200 dark:border-gray-700 mt-2">
          ...
          <a href="{{ route('productCopyIndex') }}" class="text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-white">
            Annuleren
          </a>
        </div>
      </form>
  ...
  <div class="space-y-4">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
      ...
      <select name="is_published" form="copy-edit-form"
        ...
        <option value="0" {{ old('is_published', $productCopy->is_published) == 0 ? 'selected' : '' }}>Concept</option>
        <option value="1" {{ old('is_published', $productCopy->is_published) == 1 ? 'selected' : '' }}>Gepubliceerd</option>
      </select>
      <p class="text-xs text-gray-400 dark:text-gray-500">Concept-exemplaren zijn niet zichtbaar bij producten.</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
      ...
      <dl class="space-y-2 text-sm">
        <div class="flex justify-between">
          ...
          <dd class="font-medium text-gray-900 dark:text-white">{{ $productCopy->id }}</dd>
        </div>
        <div class="flex justify-between">
          <dt class="text-gray-500 dark:text-gray-400">Aangemaakt</dt>
          <dd class="font-medium text-gray-900 dark:text-white">{{ $productCopy->created_at->format('d-m-Y') }}</dd>
        </div>
        <div class="flex justify-between">
          <dt class="text-gray-500 dark:text-gray-400">Bijgewerkt</dt>
          <dd class="font-medium text-gray-900 dark:text-white">{{ $productCopy->updated_at->format('d-m-Y') }}</dd>
        </div>
      </dl>
    </div>
  </div>
