<x-dashboard-layout>
    <main class="container page dashboard">
        <div style="margin-bottom: 2rem;">
            <a href="{{ route('newsletter.campaigns.index') }}" class="btn" style="background: #6c757d; color: white;">
                <i class="fa-solid fa-arrow-left"></i> Terug
            </a>
        </div>

        <h2 style="margin-bottom: 2rem;">Nieuwe Nieuwsbrief</h2>

        <form action="{{ route('newsletter.campaigns.store') }}" method="POST" style="background: white; padding: 2rem; border-radius: 8px;">
            @csrf
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <textarea
                    id="content"
                    name="content"
                    style="width: 100%; min-height: 400px;">{{ old('content') }}</textarea>
                @error('content')
                    <span style="color: #dc3545; font-size: 0.875rem;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="display: flex; gap: 1rem;">
                <button type="submit" class="btn btn-primary">
                    <i class="fa-solid fa-save"></i> Opslaan als concept
                </button>
                <a href="{{ route('newsletter.campaigns.index') }}" class="btn" style="background: #6c757d; color: white;">
                    Annuleren
                </a>
